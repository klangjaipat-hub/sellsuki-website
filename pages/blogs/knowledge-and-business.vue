<template>
    <div id="blog">
        ...
        <div class="container font-df ps-3 pe-3 ps-md-0 pe-md-0">
            ...
            <div class="row mt-4">
                <div class="col-12">
                    <span class="text-s-44 text-w-500 font-md d-none d-xl-block">{{ $t("Blog.Knowledge_Business") }}</span>
                    <span class="text-s-36 text-w-500 font-md d-block d-xl-none">{{ $t("Blog.Knowledge_Business") }}</span>
                </div>
            </div>
            <div class="row mt-2 d-none d-md-flex">
                <div class="col-12">
                    <div style="display: flex;justify-content: space-between;">
                        ...
                        <img @click="scrollToStart()" width="18" class="cursor-pointer" src="../../assets/icon/chevron-l.svg" alt="chevron-l.svg" style="position: relative;z-index: 999;margin-bottom: -78px;margin-left: -10px;">
                        <ul class="items list__wrapper">
                            <li class="item item1 list__item pr-2" id="1">
                                <div @click="number = '1',getBycateFilter(10)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '1'}" class="btn-df"><span class="font-df text-s-24 text-w-700">{{ $t("Blog.Market_Place") }}</span></div>
                            </li>
                            <li class="item item1 list__item pr-2" id="2">
                                <div @click="number = '2',getBycateFilter(11)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '2'}" class="btn-df"><span class="font-df text-s-24 text-w-700">{{ $t("Blog.E_Commerce") }}</span></div>
                            </li>
                            <li class="item item1 list__item pr-2" id="3">
                                <div @click="number = '3',getBycateFilter(9)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '3'}" class="btn-df"><span class="font-df text-s-24 text-w-700">{{ $t("Blog.Social_Media") }}</span></div>
                            </li>
                            <li class="item item1 list__item pr-2" id="4">
                                <div @click="number = '4',getBycateFilter(14)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '4'}" class="btn-df"><span class="font-df text-s-24 text-w-700">{{ $t("Blog.Digital_Transformation") }}</span></div>
                            </li>
                            <li class="item item1 list__item pr-2" id="5">
                                <div @click="number = '5',getBycateFilter(13)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '5'}" class="btn-df"><span class="font-df text-s-24 text-w-700">{{ $t("Blog.Martech") }}</span></div>
                            </li>
                            <li class="item item1 list__item pr-2" id="6">
                                <div @click="number = '6',getBycateFilter(8)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '6'}" class="btn-df"><span class="font-df text-s-24 text-w-700">{{ $t("Blog.Sales_Advertising") }}</span></div>
                            </li>
                            <li class="item item1 list__item pr-2" id="7">
                                <div @click="number = '7',getBycateFilter(16)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '7'}" class="btn-df"><span class="font-df text-s-24 text-w-700">{{ $t("Blog.Data_Research") }}</span></div>
                            </li>
                            <li class="item item1 list__item pr-2" id="8">
                                <div @click="number = '8',getBycateFilter(12)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '8'}" class="btn-df"><span class="font-df text-s-24 text-w-700">ครบจบ ทุกเรื่อง LINE</span></div>
                            </li>
                            <li class="item item1 list__item pr-2" id="9">
                                <div @click="number = '9',getBycateFilter(15)" :class="{'border-B3E6FF color-2DA3FD bg-EBF8FF':number == '9'}" class="btn-df"><span class="font-df text-s-24 text-w-700">{{ $t("Blog.Other") }}</span></div>
                            </li>
                        </ul>
                        <img @click="scrollToEnd()" width="18" class="cursor-pointer" src="../../assets/icon/chevron-r.svg" alt="chevron-r.svg" style="position: relative;z-index: 999;margin-top: -37px;margin-right: -20px;float: inline-end;">
                    </main>
                    ...
                    <input @click="showSearchXl()" @blur="cancelSearchXl()" v-model="search" class="form-transition font-bd" type="text" :placeholder="$t('Blog.Search')">
                    </Form>
                    </div>
                </div>
            </div>
            <div class="row mt-2 d-flex d-md-none">
                <div class="col-12 font-df">
                    ...
                    <select id="form-select" @click="isActive = !isActive" @blur="isActive = !isActive" @change="getBycateFilter(catNumber)" v-model="catNumber" class="form-select" :class="{ 'icon-select-down' : isActive == true,'icon-select-up' : isActive == false }" placeholder="interested about*">
                        <option class="color-09193A text-w-700" selected value="">{{$t("form.interestedabout")}}*</option>
                        <option class="color-09193A text-w-700" value="10">{{ $t("Blog.Market_Place") }}</option>
                        <option class="color-09193A text-w-700" value="11">{{ $t("Blog.E_Commerce") }}</option>
                        <option class="color-09193A text-w-700" value="9">{{ $t("Blog.LINE_Agency") }}</option>
                        <option class="color-09193A text-w-700" value="14">{{ $t("Blog.Digital_Transformation") }}</option>
                        <option class="color-09193A text-w-700" value="13">{{ $t("Blog.Martech") }}</option>
                        <option class="color-09193A text-w-700" value="8">{{ $t("Blog.Sales_Advertising") }}</option>
                        <option class="color-09193A text-w-700" value="16">{{ $t("Blog.Data_Research") }}</option>
                        <option class="color-09193A text-w-700" value="12">ครบจบ ทุกเรื่อง LINE</option>
                        <option class="color-09193A text-w-700" value="15">{{ $t("Blog.Other") }}</option>
                    </select>
                        ...
                        <nuxt-link v-if="index <= sumBlog" @click="scrollToTop" :to="localePath({ path: '/blogs/detail/'+data.id+'/'+data.attributes.url+''})" style="color:unset !important">
                            <div class="card" style="margin-bottom: 32px;">
                                ...
                                <div class="card-body pt-2 il-n">
                                    ...
                                    <p class="card-title text-w-500 color-1F2937 text-limit-2">{{data.attributes.title}}</p>
    ...
    export default {
        ...
        async asyncData({ $axios, route, i18n }) {
                return { responseData: [], blogHighlight: [] };
            },
        data () {
            return {
                isActive: true,
        ...
        mounted () {
            ...
            this.catNumber = 10;
            this.responseDataByCate = this.getBycate(10)
